...
import { 
  ...
  TrendingUp,
  TrendingDown
} from 'lucide-react'; // <-- CORRIGIDO A SINTAXE DE FECHAMENTO
import { 
  ...
  ResponsiveContainer,
  Pie
} from 'recharts'; // <-- CORRIGIDO A SINTAXE DE FECHAMENTO
// Assumindo que os tipos e utilitários estão corretamente definidos
import { FormData, CalculatedData } from '@/lib/types'; 
import { formatCurrency, formatPercentageDirect, normalizeNumber } from '@/lib/utils';
import { Badge } from '@/components/ui/badge'; // <-- CORRIGIDO A SINTAXE DE FECHAMENTO
...
export default function DetailedAnalysis({ 
  ...
  // Acesso seguro para evitar erros de tipagem, assumindo que `additionalValues` existe em `FormData`
  const additionalValues = formData.additionalValues || {}; 

  const handleInputChange = (field: string, value: number) => {
    // Corrigido para acessar o caminho completo do campo, como você fez na declaração da função
    onUpdateFormData(`additionalValues.${field}`, value); 
  };

  // Cálculos da análise detalhada
  const promocoes = normalizeNumber(additionalValues.promocoes || 0);
  const taxasComissoes = normalizeNumber(additionalValues.taxasComissoes || 0);
  const servicosLogisticos = normalizeNumber(additionalValues.servicosLogisticos || 0);
  const outrosValores = normalizeNumber(additionalValues.outrosValores || 0);
  ...
  // Acesso seguro a formData.vrlj
  const repasseLiquidoReal = rbrPosDebitos + normalizeNumber(formData.vrlj || 0); 
  ...
  const barData = [
    ...
    { name: 'VRLJ', value: normalizeNumber(formData.vrlj || 0), color: '#8b5cf6' },
      ...
      {!hasRbrError && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-200">
            ...
            <div className="text-xl font-bold text-purple-800">{formatCurrency(normalizeNumber(formData.vrlj || 0))}</div>
          </div>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <RechartsPieChart>
                    ...
                    <Tooltip 
                      // CORRIGIDO: Garantindo que o valor seja formatado corretamente com '%'
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <YAxis 
                    tick={{ fontSize: 12 }}
                    // CORRIGIDO: Usando template literal para a string
                  ...
                  <Tooltip 
                    ...
                    labelStyle={{ color: '#374151' }}
                  />
                  <Bar dataKey="value" fill="#3b82f6" key="bar-comparativo" /> 
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
